...
int main(int, char *[]) {
    ...
    Shader lampShader("../shaders/PN-MVP.vert", "../shaders/lampColored.frag");
    ...
    while (!quit) {
        ...
        glm::vec3 lightColor( sin(my_context.time() * 2.0f), sin(my_context.time() * 0.7f), sin(my_context.time() * 1.3f));
